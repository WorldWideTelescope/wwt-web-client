...
//Static methods
  ...
  public static double EclipticLongitude(double JD)
  {
	...
	//Calculate Longitude
	double L = 0;
    int nPlutoCoefficients = GFX.g_PlutoArgumentCoefficients.Length;
	for (int i =0; i<nPlutoCoefficients; i++)
	{
	  double Alpha = GFX.g_PlutoArgumentCoefficients[i].J * J + GFX.g_PlutoArgumentCoefficients[i].S * S + GFX.g_PlutoArgumentCoefficients[i].P * P;
	  Alpha = CT.D2R(Alpha);
...
{
    public PlutoCoefficient1(double j, double s, double p)
    {
        J = j;
        S = s;
        P = p;
...
{
    public PlutoCoefficient2(double a, double b)
    {
        A = a;
        B = b;
    }
