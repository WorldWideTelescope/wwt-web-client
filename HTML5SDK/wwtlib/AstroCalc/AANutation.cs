using System;
public static  partial class GFX
{

	public static NUC[] g_NuC = { new NUC(0, 0, 0, 0, 1, -171996, -174.2, 92025, 8.9), new NUC(-2, 0, 0, 2, 2, -13187, -1.6, 5736, -3.1), new NUC(0, 0, 0, 2, 2, -2274, -0.2, 977, -0.5), new NUC(0, 0, 0, 0, 2, 2062, 0.2, -895, 0.5), new NUC(0, 1, 0, 0, 0, 1426, -3.4, 54, -0.1), new NUC(0, 0, 1, 0, 0, 712, 0.1, -7, 0), new NUC(-2, 1, 0, 2, 2, -517, 1.2, 224, -0.6), new NUC(0, 0, 0, 2, 1, -386, -0.4, 200, 0), new NUC(0, 0, 1, 2, 2, -301, 0, 129, -0.1), new NUC(-2, -1, 0, 2, 2, 217, -0.5, -95, 0.3), new NUC(-2, 0, 1, 0, 0, -158, 0, 0, 0), new NUC(-2, 0, 0, 2, 1, 129, 0.1, -70, 0), new NUC(0, 0, -1, 2, 2, 123, 0, -53, 0), new NUC(2, 0, 0, 0, 0, 63, 0, 0, 0), new NUC(0, 0, 1, 0, 1, 63, 0.1, -33, 0), new NUC(2, 0, -1, 2, 2, -59, 0, 26, 0), new NUC(0, 0, -1, 0, 1, -58, -0.1, 32, 0), new NUC(0, 0, 1, 2, 1, -51, 0, 27, 0), new NUC(-2, 0, 2, 0, 0, 48, 0, 0, 0), new NUC(0, 0, -2, 2, 1, 46, 0, -24, 0), new NUC(2, 0, 0, 2, 2, -38, 0, 16, 0), new NUC(0, 0, 2, 2, 2, -31, 0, 13, 0), new NUC(0, 0, 2, 0, 0, 29, 0, 0, 0), new NUC(-2, 0, 1, 2, 2, 29, 0, -12, 0), new NUC(0, 0, 0, 2, 0, 26, 0, 0, 0), new NUC(-2, 0, 0, 2, 0, -22, 0, 0, 0), new NUC(0, 0, -1, 2, 1, 21, 0, -10, 0), new NUC(0, 2, 0, 0, 0, 17, -0.1, 0, 0), new NUC(2, 0, -1, 0, 1, 16, 0, -8, 0), new NUC(-2, 2, 0, 2, 2, -16, 0.1, 7, 0), new NUC(0, 1, 0, 0, 1, -15, 0, 9, 0), new NUC(-2, 0, 1, 0, 1, -13, 0, 7, 0), new NUC(0, -1, 0, 0, 1, -12, 0, 6, 0), new NUC(0, 0, 2, -2, 0, 11, 0, 0, 0), new NUC(2, 0, -1, 2, 1, -10, 0, 5, 0), new NUC(2, 0, 1, 2, 2, -8, 0, 3, 0), new NUC(0, 1, 0, 2, 2, 7, 0, -3, 0), new NUC(-2, 1, 1, 0, 0, -7, 0, 0, 0), new NUC(0, -1, 0, 2, 2, -7, 0, 3, 0), new NUC(2, 0, 0, 2, 1, -7, 0, 3, 0), new NUC(2, 0, 1, 0, 0, 6, 0, 0, 0), new NUC(-2, 0, 2, 2, 2, 6, 0, -3, 0), new NUC(-2, 0, 1, 2, 1, 6, 0, -3, 0), new NUC(2, 0, -2, 0, 1, -6, 0, 3, 0), new NUC(2, 0, 0, 0, 1, -6, 0, 3, 0), new NUC(0, -1, 1, 0, 0, 5, 0, 0, 0), new NUC(-2, -1, 0, 2, 1, -5, 0, 3, 0), new NUC(-2, 0, 0, 0, 1, -5, 0, 3, 0), new NUC(0, 0, 2, 2, 1, -5, 0, 3, 0), new NUC(-2, 0, 2, 0, 1, 4, 0, 0, 0), new NUC(-2, 1, 0, 2, 1, 4, 0, 0, 0), new NUC(0, 0, 1, -2, 0, 4, 0, 0, 0), new NUC(-1, 0, 1, 0, 0, -4, 0, 0, 0), new NUC(-2, 1, 0, 0, 0, -4, 0, 0, 0), new NUC(1, 0, 0, 0, 0, -4, 0, 0, 0), new NUC(0, 0, 1, 2, 0, 3, 0, 0, 0), new NUC(0, 0, -2, 2, 2, -3, 0, 0, 0), new NUC(-1, -1, 1, 0, 0, -3, 0, 0, 0), new NUC(0, 1, 1, 0, 0, -3, 0, 0, 0), new NUC(0, -1, 1, 2, 2, -3, 0, 0, 0), new NUC(2, -1, -1, 2, 2, -3, 0, 0, 0), new NUC(0, 0, 3, 2, 2, -3, 0, 0, 0), new NUC(2, -1, 0, 2, 2, -3, 0, 0, 0) };
}
//
//Module : AANUTATION.CPP
//Purpose: Implementation for the algorithms for Nutation
//Created: PJN / 29-12-2003
//History: None
//
//Copyright (c) 2003 - 2007 by PJ Naughter (Web: www.naughter.com, Email: pjna@naughter.com)
//
//All rights reserved.
//
//Copyright / Usage Details:
//
//You are allowed to include the source code in any product (commercial, shareware, freeware or otherwise) 
//when your product is released in binary form. You are allowed to modify the source code in any way you want 
//except you cannot modify the copyright details at the top of each module. If you want to distribute source 
//code with your application, then you are only allowed to distribute versions released by the author. This is 
//to maintain a single distribution point for the source code. 
//
//


//////////////////////////// Includes /////////////////////////////////////////



/////////////////////// Classes ///////////////////////////////////////////////

public class  CAANutation
{
//Static methods

  ///////////////////////////// Implementation //////////////////////////////////
  
  public static double NutationInLongitude(double JD)
  {
	double T = (JD - 2451545) / 36525;
	double Tsquared = T *T;
	double Tcubed = Tsquared *T;
  
	double D = 297.85036 + 445267.111480 *T - 0.0019142 *Tsquared + Tcubed / 189474;
	D = CT.M360(D);
  
	double M = 357.52772 + 35999.050340 *T - 0.0001603 *Tsquared - Tcubed / 300000;
	M = CT.M360(M);
  
	double Mprime = 134.96298 + 477198.867398 *T + 0.0086972 *Tsquared + Tcubed / 56250;
	Mprime = CT.M360(Mprime);
  
	double F = 93.27191 + 483202.017538 *T - 0.0036825 *Tsquared + Tcubed / 327270;
	F = CT.M360(F);
  
	double omega = 125.04452 - 1934.136261 *T + 0.0020708 *Tsquared + Tcubed / 450000;
	omega = CT.M360(omega);

    int nCoefficients = GFX.g_NuC.Length;
	double @value = 0;
	for (int i =0; i<nCoefficients; i++)
	{
	  double argument = GFX.g_NuC[i].D * D + GFX.g_NuC[i].M * M + GFX.g_NuC[i].Mprime * Mprime + GFX.g_NuC[i].F * F + GFX.g_NuC[i].omega * omega;
	  double radargument = CT.D2R(argument);
	  @value += (GFX.g_NuC[i].sincoeff1 + GFX.g_NuC[i].sincoeff2 * T) * Math.Sin(radargument) * 0.0001;
	}
  
	return @value;
  }
	public static double NutationInObliquity(double JD)
	{
	  double T = (JD - 2451545) / 36525;
	  double Tsquared = T *T;
	  double Tcubed = Tsquared *T;
	
	  double D = 297.85036 + 445267.111480 *T - 0.0019142 *Tsquared + Tcubed / 189474;
	  D = CT.M360(D);
	
	  double M = 357.52772 + 35999.050340 *T - 0.0001603 *Tsquared - Tcubed / 300000;
	  M = CT.M360(M);
	
	  double Mprime = 134.96298 + 477198.867398 *T + 0.0086972 *Tsquared + Tcubed / 56250;
	  Mprime = CT.M360(Mprime);
	
	  double F = 93.27191 + 483202.017538 *T - 0.0036825 *Tsquared + Tcubed / 327270;
	  F = CT.M360(F);
	
	  double omega = 125.04452 - 1934.136261 *T + 0.0020708 *Tsquared + Tcubed / 450000;
	  omega = CT.M360(omega);

      int nCoefficients = GFX.g_NuC.Length;
	  double @value = 0;
	  for (int i =0; i<nCoefficients; i++)
	  {
		double argument = GFX.g_NuC[i].D * D + GFX.g_NuC[i].M * M + GFX.g_NuC[i].Mprime * Mprime + GFX.g_NuC[i].F * F + GFX.g_NuC[i].omega * omega;
		double radargument = CT.D2R(argument);
		@value += (GFX.g_NuC[i].coscoeff1 + GFX.g_NuC[i].coscoeff2 * T) * Math.Cos(radargument) * 0.0001;
	  }
	
	  return @value;
	}
  public static double NutationInRightAscension(double Alpha, double Delta, double Obliquity, double NutationInLongitude, double NutationInObliquity)
  {
	//Convert to radians
	Alpha = CT.H2R(Alpha);
	Delta = CT.D2R(Delta);
	Obliquity = CT.D2R(Obliquity);
  
	return (Math.Cos(Obliquity) + Math.Sin(Obliquity) * Math.Sin(Alpha) * Math.Tan(Delta)) * NutationInLongitude - Math.Cos(Alpha)*Math.Tan(Delta)*NutationInObliquity;
  }
  public static double NutationInDeclination(double Alpha, double Delta, double Obliquity, double NutationInLongitude, double NutationInObliquity)
  {
	//Convert to radians
	Alpha = CT.H2R(Alpha);
	Delta = CT.D2R(Delta);
	Obliquity = CT.D2R(Obliquity);
  
	return Math.Sin(Obliquity) * Math.Cos(Alpha) * NutationInLongitude + Math.Sin(Alpha)*NutationInObliquity;
  }
  public static double MeanObliquityOfEcliptic(double JD)
  {
	double U = (JD - 2451545) / 3652500;
	double Usquared = U *U;
	double Ucubed = Usquared *U;
	double U4 = Ucubed *U;
	double U5 = U4 *U;
	double U6 = U5 *U;
	double U7 = U6 *U;
	double U8 = U7 *U;
	double U9 = U8 *U;
	double U10 = U9 *U;
  
  
	return CT.DMS2D(23, 26, 21.448) - CT.DMS2D(0, 0, 4680.93) * U - CT.DMS2D(0, 0, 1.55) * Usquared + CT.DMS2D(0, 0, 1999.25) * Ucubed - CT.DMS2D(0, 0, 51.38) * U4 - CT.DMS2D(0, 0, 249.67) * U5 - CT.DMS2D(0, 0, 39.05) * U6 + CT.DMS2D(0, 0, 7.12) * U7 + CT.DMS2D(0, 0, 27.87) * U8 + CT.DMS2D(0, 0, 5.79) * U9 + CT.DMS2D(0, 0, 2.45) * U10;
  }
  public static double TrueObliquityOfEcliptic(double JD)
  {
	return MeanObliquityOfEcliptic(JD) + CT.DMS2D(0, 0, NutationInObliquity(JD));
  }

}


//////////////////////////// Macros / Defines /////////////////////////////////

public class NUC // was NutationCoefficient
{
    public NUC(int D, int M, int Mprime, int F, int omega, int sincoeff1, double sincoeff2, int coscoeff1, double coscoeff2)
    {
        this.D = D;
        this.M = M;
        this.Mprime = Mprime;
        this.F = F;
        this.omega = omega;
        this.sincoeff1 = sincoeff1;
        this.sincoeff2 = sincoeff2;
        this.coscoeff1 = coscoeff1;
        this.coscoeff2 = coscoeff2;
    }
    public int D;
    public int M;
    public int Mprime;
    public int F;
    public int omega;
    public int sincoeff1;
    public double sincoeff2;
    public int coscoeff1;
    public double coscoeff2;

}
